...
class UserController extends Controller
{
    ...
    public function user_dashboard(Request $request){
        ...
        $restaurantNearbyyou = $this->loginRepository->getRestaurantDetailnearYou($live_lattitude, $live_longitude,$distance = 5);
    ...
    public function add_to_cart(Request $request){
        ...
        if ($validator->fails()) {
            ...
            if($checkCartdataAdd){
                ...
                $insert = $this->usercartRepository->updateCartdata($totalQty, $totalPrice, $checkCartdataAdd->id);
            }else{
    ...
    public function user_order_book_table(Request $request){
        ...
        if ($validator->fails()) {
            ...
            if ($insert) {
                ...
                $msg = 'Reservation at '.$resname.' '.$datetime.' for '.$guest.' guests at '. $gettableTime->time_from.'';
                $notifications = self::sendNotification($did, $title, $msg, '2', $request->book_table_id);

                
                $succesNotes = 'For ' . $request->number_of_people.', '. date('d,M', strtotime($request->booking_date)) . ' at ' . $gettableTime->time_from;
                return response()->json(['error_msg' => 'Table is booked.', 'status' => 1, 'data' => array(array('id' => $insert->id, 'successnote' => $succesNotes))], $this->successStatus);
            } else {
                return response()->json(['error_msg' => 'Something went wrong.', 'data' => array(), 'status' => 0], $this->successStatus);
            }
    ...
    public function cancel_booking(Request $request){
        ...
        if ($validator->fails()) {
            ...
            $currentDate = date('Y-m-d');
            $currentTime = date('H:i:s');
            $getTimetable = $this->userbooktableRepository->getBooktableWithdateandtime($request->id, $currentDate, $currentTime);
            if(count($getTimetable) > 0){
                $deleteBooking = Userbooktable::where('id',$request->id)->delete();
